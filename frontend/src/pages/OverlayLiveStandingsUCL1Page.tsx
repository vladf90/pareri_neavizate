/**
 * Live Standings UCL Overlay Page - Part 1 (Positions 1-16)
 * 
 * Dedicated overlay for UCL 32-team format.
 * Shows positions 1-16 (top half).
...
function TeamRow({ entry, index, qualificationLine = 8, playoffLine = 24, isHighlighted }: TeamRowProps) {
  ...
  return (
    <motion.div
      ...
      className={`relative flex items-center justify-between px-4 h-[80px] rounded-r-xl ${rowStyle}`}
    >
      ...
      <div className="flex items-center flex-1 gap-4 ml-2 min-w-0">
        ...
        <span className="text-[140px] font-bebas text-[#F5F5F5] tracking-wide leading-none drop-shadow-md truncate">
      ...
      {rank === qualificationLine && (
        <div className="absolute bottom-[-5px] left-0 w-full h-[4px] bg-[#00FF9D] shadow-[0_0_15px_#00FF9D] z-10" />
      )}
    </motion.div>
  );
}
...
function LiveStandingsWidget({ standings, qualificationLine = 8, playoffLine = 24, highlightTeamIds = [], subtitle }: LiveStandingsWidgetProps) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center p-3">
      <div className="w-full h-full bg-[#0A1B51]/40 backdrop-blur-2xl border-4 border-[#33EFFF] rounded-[32px] shadow-[0_0_80px_rgba(51,239,255,0.2)] overflow-hidden flex flex-col relative">
        
        {/* Compact Header */}
        <div className="pt-4 pb-2 px-4 flex flex-col items-center justify-center border-b-2 border-white/10 bg-gradient-to-b from-[#0A1B51]/90 to-transparent">
          <div className="w-14 h-14 mb-1 bg-white/5 rounded-full flex items-center justify-center border-2 border-[#F659FD] shadow-[0_0_20px_rgba(246,89,253,0.4)] overflow-hidden">
            <img src={logoImage} className="w-10 h-10 object-contain" alt="Pareri Neavizate" />
          </div>

          <h1 className="text-[64px] font-bebas text-white tracking-widest text-center leading-[0.85] drop-shadow-xl">
            LIVE STANDINGS
          </h1>
          <span className="text-lg font-montserrat font-black text-[#33EFFF] tracking-[0.3em] uppercase drop-shadow-md">
            {standings.competitionName || "Champions League"}
          </span>
          {subtitle && (
            <span className="text-base font-montserrat font-bold text-[#F659FD] tracking-[0.2em] uppercase mt-0.5">
              {subtitle}
            </span>
          )}
        </div>

        {/* Body - 16 teams, no scroll */}
        <div className="flex-1 overflow-hidden px-3 py-2 flex flex-col gap-[2px]">
        ...
        {/* Compact Footer */}
        <div className="h-14 border-t-2 border-white/10 flex flex-col items-center justify-center bg-[#0A1B51]/80">
          <span className="text-xs uppercase tracking-[0.2em] text-[#F5F5F5]/50 font-bold">
            Brought to you by
          </span>
          <span className="text-2xl font-bebas text-[#F659FD] tracking-widest drop-shadow-[0_0_10px_rgba(246,89,253,0.5)]">
            SUPERBET
          </span>
        </div>
...
/**
 * Live Standings UCL Part 1 - Positions 1-16
 */
export function OverlayLiveStandingsUCL1Page() {
  const { state } = useOverlayConnection({ overlayName: "livestandings-ucl1" });
  ...
  const segmentedStandings = {
    ...standings,
    entries: sortedEntries.slice(0, 16)
  };
